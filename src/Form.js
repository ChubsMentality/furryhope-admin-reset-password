...
const Form = () => {
    ...
    const verifyToken = async () => {
        try {
            const { data } = await axios.get(`${url}api/admins/verify-token?token=${token}&id=${id}`)
            setLoading(false)
            console.log(data)
        } catch (error) {
    ...
    const changePassword = async (e) => {
        ...
        try {
            const { data } = await axios.post(`${url}api/admins/resetPassword?token=${token}&id=${id}`, { password })
            setLoading(false)
            console.log(data)
